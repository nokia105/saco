...
<div class="container">
  ...
  <div style="float:right; margin-bottom:20px;">
    Invoice #: {{$loan->loanpayment->mode_no}}<br>
    Created: {{date('Y-m-d')}}<br>
   </div>

   <div style="float:left; margin-bottom:20px;">
    Name:{{$loan->member->first_name}} {{$loan->member->middle_name}} {{$loan->member->last_name}}<br>
    No:{{$loan->member->registration_no}}<br>
   </div> 

  

  <div style="text-align:center; margin-bottom:30px;">
    STAKABADHI YA KUPOKEA FEDHA
  </div>   
   <div id="table">
  <table class="table table-striped table-bordered">
      ...
      @if(Request::is('paid_submitted'))
      <button><a href="{{route('paidloans_slip',$loan->id)}}">print<i class="fa fa-print"></i></a></button>
      @endif
     </div> 
    </div>
    </html>
